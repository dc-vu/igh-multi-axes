...
const static ec_pdo_entry_reg_t domain_w_regs[] = 
{
        ...
        {servo1,servo1_code,0x6041,00,&status_word            },  // mode 8
        // {servo1,servo1_code,0x6061,00,&mode_display           },
        {servo1,servo1_code,0x6064,00,&pos_act                }, // mode 8
...
ec_pdo_info_t slave_0_pdos[] = {
    ...
    {0x1a02, 4, slave_0_pdo_entries + 2}, /* 258th transmit PDO Mapping */
};
...
void cyclic_task()
{
	...
	while(1) 
	{
		...
		else if( (temp[0]&0x006f) == 0x0023)
		{
            ...
            EC_WRITE_S16(domain_r_pd+tar_torq, offsets);
        ...
        else if( (temp[0]&0x006f) == 0x0027)        // this servo is on
		{
            ...
            EC_WRITE_S16(domain_r_pd+tar_torq, move_value);
...
// #if SDO_ACCESS
    ...
    if (ecrt_slave_config_sdo8(sc, 0x6060, 0, 10))
    {
        return -1;
    }
