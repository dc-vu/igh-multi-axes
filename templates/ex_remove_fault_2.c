...
uint8_t fault_resetted = 0;

// offsets for PDO entries
float move_value1 = 0;
float move_value2 = 0;
static unsigned int count_reset = 0;
static unsigned int error_flag = 1;
...
void cyclic_task()
{
    ...
    while(fault_resetted == 0) {
        ...
        // if ((error_flag == 1) & ((temp[1] & 0x006f) == 0x0028) & (t_fault > 0.5))
        // {
        //     EC_WRITE_U16(domain1_pd + off_controlword1, 0b00001111);
        //     error_flag == 0;
        //     fault_resetted = 1;
        // }
        // else  if (((temp[1] & 0x006f) == 0x0028) & (error_flag == 1 )) {
        //     // Ready to switch on
        //     EC_WRITE_U16(domain1_pd + off_controlword1, 0b10001111);
        //     t_fault += 0.001;
        // }
        ...
        // write process data
        if (((temp[1] & 0x007f) == 0x0038) & (count_reset < 100))
        {
            printf("reset-------------------------------------\n");
            EC_WRITE_U16(domain1_pd + off_controlword1, 0b10001111);
            count_reset += 1;
        }
        else if (((temp[1] & 0x007f) == 0x0038) & (count_reset == 100))
        {
            EC_WRITE_U16(domain1_pd + off_controlword1, 0b00001111);
            count_reset = 0;
        }
