...
uint8_t temp[12]={};
uint8_t servo_status[num_servo] = {0, 0, 0, 0, 0, 0};

static unsigned int ctrl_word[6];
static unsigned int tar_torque[6];
static unsigned int status_word[6];
static unsigned int torque_actual[6];

//rx pdo entry 
// RxPDO for Torque Mode
const static ec_pdo_entry_reg_t domain_r_regs[] = 
{
    {servo1, servo1_code, 0x6040, 0, &ctrl_word[0]},
    {servo2, servo2_code, 0x6040, 0, &ctrl_word[1]},
    {servo3, servo3_code, 0x6040, 0, &ctrl_word[2]},
    {servo4, servo4_code, 0x6040, 0, &ctrl_word[3]},
    {servo5, servo5_code, 0x6040, 0, &ctrl_word[4]},
    {servo6, servo6_code, 0x6040, 0, &ctrl_word[5]},
    
    // Optional but recommended for switching mode
    // You can define a 'mode' variable, e.g. int8_t mode[6];
    // {servo1, servo1_code, 0x6060, 0, &mode[0]},

    {servo1, servo1_code, 0x6071, 0, &tar_torque[0]},
    {servo2, servo2_code, 0x6071, 0, &tar_torque[1]},
    {servo3, servo3_code, 0x6071, 0, &tar_torque[2]},
    {servo4, servo4_code, 0x6071, 0, &tar_torque[3]},
    {servo5, servo5_code, 0x6071, 0, &tar_torque[4]},
    {servo6, servo6_code, 0x6071, 0, &tar_torque[5]},

    {}
};


// TxPDO for Torque Mode
const static ec_pdo_entry_reg_t domain_w_regs[] = 
{
    {servo1, servo1_code, 0x6041, 0, &status_word[0]},
    {servo2, servo2_code, 0x6041, 0, &status_word[1]},
    {servo3, servo3_code, 0x6041, 0, &status_word[2]},
    {servo4, servo4_code, 0x6041, 0, &status_word[3]},
    {servo5, servo5_code, 0x6041, 0, &status_word[4]},
    {servo6, servo6_code, 0x6041, 0, &status_word[5]},

    {servo1, servo1_code, 0x6077, 0, &torque_actual[0]},
    {servo2, servo2_code, 0x6077, 0, &torque_actual[1]},
    {servo3, servo3_code, 0x6077, 0, &torque_actual[2]},
    {servo4, servo4_code, 0x6077, 0, &torque_actual[3]},
    {servo5, servo5_code, 0x6077, 0, &torque_actual[4]},
    {servo6, servo6_code, 0x6077, 0, &torque_actual[5]},
...
//     {0x1702, 4, slave_0_pdo_entries + 0},  /* RxPDO: Controlword + Target torque + Mode + Touch probe */
//     {0x1b02, 8, slave_0_pdo_entries + 4},  /* TxPDO: các phản hồi */
// };
...
ec_pdo_info_t slave_0_pdos[] = {
    {0x1702, 7, slave_0_pdo_entries + 0},  /* RxPDO: Controlword + Target torque + Mode + Touch probe */
    {0x1b02, 9, slave_0_pdo_entries + 7},  /* TxPDO: các phản hồi */
};
...
void cyclic_task()
{
	...
	while(1) 
	{
		...
		{ // do this at 1 Hz
            ...
            for (int i = 0; i < 6; i++) 
            {
                uint16_t val = EC_READ_U16(domain_w_pd + status_word[i]);
                temp[2 * i]     = val & 0xFF;        // LSB
                temp[2 * i + 1] = (val >> 8) & 0xFF; // MSB

                printf("Status[%d] = 0x%02X%02X        ", i, temp[2 * i + 1], temp[2 * i]);  // MSB:LSB
            }
            printf("\n============================\n");

            for (int i = 0; i < num_servo; i++) 
            {
                printf("Servo %d status: %d         ", i + 1, servo_status[i]);
            }
            printf("\n");
        ...
        for (int i=0; i< num_servo; i++)
        {
            if ( (temp[i] & 0b00001000) == 0b00001000 )
            {
                ...
                servo_status[i] = 0;
                // printf("Servo %d is in fault state\n", i+1);
            }
            else if((temp[i]&0x006f) == 0x0060 )
            {
                servo_ON[i] = 0;
                EC_WRITE_U16(domain_r_pd+ctrl_word[i], 0x0006);
                servo_status[i] = 1;
                // printf("Servo %d is deactivated\n", i+1);   
            }
            else if( (temp[i]&0x006f) == 0x0040  )
            {
                servo_ON[i] = 0;
                EC_WRITE_U16(domain_r_pd+ctrl_word[i], 0x0006);
                servo_status[i] = 2;
                // printf("Servo %d is in switch on state\n", i+1);
            }
            ...
            else if( (temp[i]&0x006f) == 0x0023)
            {
                ...
                // EC_WRITE_S32(domain_r_pd+tar_torque[i],0);
                servo_status[i] = 4;
                // printf("Servo %d is in operation enabled state\n", i+1);
            }
            else if( (temp[i]&0x006f) == 0x0027)        // this servo is on
            {
                servo_ON[i] = 1;
                EC_WRITE_U16(domain_r_pd+ctrl_word[i], 0x001f);
                servo_status[i] = 5;
                // printf("Servo %d is in operation enabled state\n", i+1);
            }
            else
            {
                servo_ON[i] = 0;
                servo_status[i] = 6;
                // printf("Servo %d is in unknown state\n", i+1);
            }
        ...
        // if ((servo4_ON == 1))
        {
            ...
            EC_WRITE_S32(domain_r_pd+tar_torque[0], move_value); // set target position
            EC_WRITE_S32(domain_r_pd+tar_torque[1], move_value); // set target position
            EC_WRITE_S32(domain_r_pd+tar_torque[2], -move_value); // set target position
            EC_WRITE_S32(domain_r_pd+tar_torque[3], -move_value); // set target position
            EC_WRITE_S32(domain_r_pd+tar_torque[4], move_value); // set target position
            EC_WRITE_S32(domain_r_pd+tar_torque[5], move_value); // set target position
...
int main(int argc, char **argv)
{
    ...
    if (ecrt_slave_config_sdo8(sc1, 0x6060, 0, 10))
    {
        return -1;
    }
    
    if (ecrt_slave_config_sdo16(sc1, 0x1C12, 01, 0x1702))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc1, 0x1C13, 01, 0x1B02))
    {
        return -1;
    }
    ...
    if (ecrt_slave_config_sdo8(sc2, 0x6060, 0, 10))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc2, 0x1C12, 01, 0x1702))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc2, 0x1C13, 01, 0x1B02))
    {
        return -1;
    }
    ...
    if (ecrt_slave_config_sdo8(sc3, 0x6060, 0, 10))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc3, 0x1C12, 01, 0x1702))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc3, 0x1C13, 01, 0x1B02))
    {
        return -1;
    }
    ...
    if (ecrt_slave_config_sdo8(sc4, 0x6060, 0, 10))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc4, 0x1C12, 01, 0x1702))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc4, 0x1C13, 01, 0x1B02))
    {
        return -1;
    }
    ...
    if (ecrt_slave_config_sdo8(sc5, 0x6060, 0, 10))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc5, 0x1C12, 01, 0x1702))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc5, 0x1C13, 01, 0x1B02))
    {
        return -1;
    }
    ...
    if (ecrt_slave_config_sdo8(sc6, 0x6060, 0, 10))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc6, 0x1C12, 01, 0x1702))
    {
        return -1;
    }

    if (ecrt_slave_config_sdo16(sc6, 0x1C13, 01, 0x1B02))
    {
        return -1;
    }
